...
describe("supervisor planner status", () => {
  test("keeps durable multi-Goal state separate from exact operational observations", async () => {
    ...
    const first = await createGoal({ cwd: repository.root, title: "First", outcome: "First outcome.", approval: "Approved." });
    const second = await createGoal({ cwd: repository.root, title: "Second", outcome: "Second outcome.", approval: "Approved." });
    ...
    const status = await deriveSupervisorPlannerStatus(repository.root, herdr);
  ...
  test("Herdr discovery failure is unavailable operational data, not durable cleanup failure", async () => {
    const repository = await temporaryRepository(); repositories.push(repository);
    const goal = await createGoal({ cwd: repository.root, title: "Only", outcome: "Preserve durable state.", approval: "Approved." });
    const herdr: HerdrOperations = {
