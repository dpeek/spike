...
Status derives the ordered queue and exact unresolved head from Application, decision, and terminal-resolution documents. A Project supervisor may instead resolve only that exact head without touching `main`: `spike application return --goal <goal> --application <id> --statement <text>` requires reported, cleanup-healthy reviewed Candidate evidence and unchanged pinned M; it records immutable M/G/Candidate/producer/highest-review evidence, invalidates approval, removes the attempt, and immediately releases Goal planning. That returned Goal may queue again only after a later landed Change advances G. `spike application stale --goal <goal> --application <id>` requires the same settled/healthy pinned-first-Ticket state and a moved `main`; it records expected and observed M plus G and any Candidate/producer, invalidates approval, removes the attempt, but keeps Goal planning frozen. A stale Goal may explicitly requeue the same G with a new Application ID and Project FIFO position; M is newly pinned only when that new attempt reaches head. Resolved attempts never reopen or regain position. Malformed resolution evidence is a visible barrier only when its own Application is reached, not a preflight scan of later queue entries. Resolution documents never update main, Goal refs, Candidate refs, or the worktree.

Only `spike application apply-head --goal <goal> --application <id>` may select `main`, and it refuses any missing, resolved, non-head, mismatched, or stale Application before creating a Candidate, decision, ref, or worktree effect. A head whose `main` still equals its Goal initial revision preserves the existing clean-base flow: it creates the deterministic sole-parent squash and advances checked-out `main` through Git. When `main` has advanced, only the exact fully-settled, cleanup-healthy, highest-approved diverged Candidate may apply. The supervisor first publishes a distinct immutable reviewed decision binding M/G/B, Candidate, producer, approving review, separate Application approval, squash form, and time; it then uses Git compare-and-swap to update only `refs/heads/main` from M to C. It never inspects, checks out, resets, merges, or rewrites the host worktree for reviewed apply.

A reviewed decision remains the FIFO owner: main at M is **decision-pending** and recovery retries CAS, while main at C is **applied** and releases the next head. Any valid decision remains historically applied when C is an ancestor of current main after later FIFO advancement. A target that is neither M nor a descendant of C is **target-mismatch** for reviewed decisions, with no mutation and required operator intervention; clean-base decisions fail closed as inconsistent. Decision absence at M needs no recovery. Status exposes these facts separately from runtime observations; there is no mutable applied flag. Goal planners and workers cannot select or mutate main. Two independently completed Goals still require separate Application approvals and deliver serially as FIFO squash commits.
...
Spike deliberately does not preflight cleanliness. Git updates the checked-out worktree or refuses conflicting local changes without Spike overwriting them. On recovery, a decision advances only an exactly unchanged expected `main`, accepts its recorded result or a current `main` descended from that result after later FIFO advancement, and refuses unrelated history. A Goal is applied only when valid decision evidence and Git history agree; intent-only and mismatched evidence are surfaced as incomplete or inconsistent. An applied Goal is terminal: no later Change or Application may be created.

Clean-base deterministic squash Application needs no model review. If `main` has advanced from the Goal base, the head may receive one supervisor-owned implementation Ticket in the separate Goal/Application/Ticket namespace. Its first Ticket pins target M, Goal revision G, merge base B, frozen model/policy, and Implement guidance loaded from the Git blob at M; later Tickets cannot repin those values. The bounded exchange exposes only declared M/G/B/input refs. A clean three-way result starts the worker from the host-computed result; a conflict starts from M with reproducible conflict evidence. After untrusted-output validation, Spike normalizes the worker tree into a one-parent-on-M Candidate and only immutable Application Report publication makes it authoritative. Retention is Report-derived at `refs/spike/goals/<goal>/applications/<application>/tickets/<ticket>`. Later target movement is visible as a mismatch and blocks further production without rewriting M or an already published Candidate. This stage never reviews, decides, or advances main.
