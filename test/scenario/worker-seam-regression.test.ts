...
async function fixture() {
  ...
  const goal = await createGoal({ cwd: repository.root, title: "Worker seam", outcome: "Keep runtime authority selected.", approval: "Approved." });
  await createChange({
    cwd: repository.root, goalId: goal.goal.metadata.goalId, title: "Validate seam", intent: "Reject false runtime evidence.",
    rationale: "Ticket policy selects the adapter.", acceptanceCriteria: ["Runtime evidence is authoritative."],
  });
  const ticket = await issueTicket({ cwd: repository.root, goalId: goal.goal.metadata.goalId, changeId: "001", instruction: "Validate.", executionPolicy: policy });
...
describe("Worker seam regressions", () => {
  test("recording ignores a name-matched lookalike and validates through the selected adapter", async () => {
    ...
    await expect(recordWorker(repository.root, {
  ...
  test("loading rejects failed host/not-launched provenance just as publication does", async () => {
    ...
    await expect(publishFailedReport({ cwd: repository.root, ...identity, role: "implement", reason: "No launch.", execution })).rejects.toThrow("permitted only");
    await installImmutable(repository.root, reportPath(repository.root, identity.goalId, identity.changeId, identity.ticketId), serializeDocument({
    ...
    await expect(loadReport(repository.root, identity.goalId, identity.changeId, identity.ticketId)).rejects.toThrow("permitted only");
  });
});
