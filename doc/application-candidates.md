...
A reviewed decision is a FIFO barrier until `main == C`: `main == M` is **decision-pending** and recovery retries the CAS; `main == C` is **applied** and releases the next unresolved head. It remains historically applied when C is an ancestor of current `main` after later FIFO advancement. A `main` that is neither M nor a descendant of C is visible as **target-mismatch** and recovery makes no mutation. No decision with `main == M` has work to recover. Candidate retention is rebuildable evidence and never defines applied state. Target mismatch requires explicit operator intervention; it is not automatically resolved.
