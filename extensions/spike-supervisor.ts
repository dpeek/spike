...
export default function spikeSupervisor(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Before dispatch_ticket, inspect both spike ticket status --json and spike run status --json. A ready ticket and its run survive supervisor restarts; never redispatch or infer a retry merely because the supervisor restarted or a live runtime cannot be found. A stopped or failed durable run can be retried only by an operator using spike run retry with the exact active run ID acknowledgement and a nonblank --reason; launch_failed still requires its exact-ID acknowledgement.",
      ...
      "A completion notification or terminal output is supporting evidence, not a durable structured report. After a durable worker reports completion, use spike run report import and spike run report show before publication or acceptance; never parse terminal prose into a report.",
      "Publish only after a persistent worker reports that it committed its intended changes, completed verification, and its imported completion report has been inspected.",
