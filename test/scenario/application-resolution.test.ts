...
async function recordImplementation(repository: Awaited<ReturnType<typeof temporaryRepository>>, goalId: string, applicationId: string, target: string, goalRevision: string, completed: boolean, reported = true) {
  ...
  if (reported) {
    await writeFile(applicationReportPath(repository.project, goalId, applicationId, "001"), serializeDocument(report, "# Report\n"));
...
async function recordReview(repository: Awaited<ReturnType<typeof temporaryRepository>>, goalId: string, applicationId: string, target: string, goalRevision: string) {
  ...
  await writeFile(applicationReviewReportPath(repository.project, goalId, applicationId, "001"), serializeDocument(reviewReport, "# Review report\n"));
}
...
async function writeCompletedChangeReport(repository: Awaited<ReturnType<typeof temporaryRepository>>, goalId: string, changeId: string, ticketId: string, metadata: Record<string, unknown>) {
...
async function fixture(completed: boolean, reported = true) {
  ...
  const goal = await createGoal({ cwd: repository.root, hostPaths: repository.hostPaths, title: "Resolution", outcome: "Keep evidence immutable.", approval: "Approved." });
  const goalId = goal.goal.metadata.goalId, base = await repository.git("rev-parse", "HEAD");
  const application = await publishApplication({ root: repository.project, goalId, integratedRevision: base, approval: "Queue." });
...
describe("Application terminal resolution", () => {
  test("records immutable stale evidence, advances FIFO, and permits same-G requeue without touching refs", async () => {
    ...
    const stale = await staleApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "001" });
    ...
    expect(await queuedApplicationHead(repository.project)).toBeUndefined();
    expect((await loadApplicationResolutionIfPresent(repository.project, goalId, "001"))?.metadata.expectedMainRevision).toBe(base);
    expect(await applicationRequeueEligibility(repository.project, goalId)).toBe("stale");
    const requeued = await queueGoalIntegration({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, approval: "Requeue stale work." });
    expect(requeued.applicationId).toBe("002"); expect(requeued.queuePosition).toBe(2);
    expect((await queuedApplicationHead(repository.project))?.metadata.applicationId).toBe("002");
    const status = await deriveGoalStatus(repository.root, repository.hostPaths, goalId);
  ...
  test("uses the latest requeued attempt so stale history cannot override a later return", async () => {
    ...
    await staleApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "001" });
    const requeued = await queueGoalIntegration({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, approval: "Requeue stale work." });
    expect(requeued.applicationId).toBe("002"); expect(requeued.queuePosition).toBe(2);
    await recordImplementation(repository, goalId, "002", moved, base, true);
    await recordReview(repository, goalId, "002", moved, base);
    await returnApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "002", statement: "Resume Goal planning." });
    const status = await deriveGoalStatus(repository.root, repository.hostPaths, goalId);
    expect(status.frozen).toBe(false); expect(status.stale).toBe(false);
    await createChange({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, title: "After stale then return", intent: "Resume product work.", rationale: "Latest returned Application releases planning.", acceptanceCriteria: ["Planning can continue."] });
  });
  ...
  test("rejects malformed reached stale evidence but does not preflight unrelated later evidence", async () => {
    ...
    await writeFile(applicationResolutionPath(repository.project, goalId, "001"), serializeDocument(malformed, "# Stale\n"));
    await expect(queuedApplicationHead(repository.project)).rejects.toThrow("malformed resolution evidence");

    const second = await publishApplication({ root: repository.project, goalId, integratedRevision: base, approval: "Later." });
    ...
    await writeFile(applicationResolutionPath(repository.project, goalId, second.metadata.applicationId), serializeDocument(unrelated, "# Stale\n"));
    ...
    const later = await publishApplication({ root: other.repository.project, goalId: other.goalId, integratedRevision: other.base, approval: "Later." });
    ...
    await writeFile(applicationResolutionPath(other.repository.project, other.goalId, later.metadata.applicationId), serializeDocument(laterMalformed, "# Stale\n"));
    expect((await queuedApplicationHead(other.repository.project))?.metadata.applicationId).toBe("001");
    ...
    await staleApplication({ cwd: provenance.repository.root, hostPaths: provenance.repository.hostPaths, goalId: provenance.goalId, applicationId: "001" });
    const reached = await publishApplication({ root: provenance.repository.project, goalId: provenance.goalId, integratedRevision: provenance.base, approval: "Later." });
    ...
    await writeFile(applicationResolutionPath(provenance.repository.project, provenance.goalId, reached.metadata.applicationId), serializeDocument(oneSided, "# Stale\n"));
    await expect(queuedApplicationHead(provenance.repository.project)).rejects.toThrow("malformed resolution evidence");
  });
...
describe("Application terminal resolution", () => {
  test("recovery skips a resolved attempt and advances the earliest later decision owner", async () => {
    ...
    const moved = await repository.git("rev-parse", "HEAD");
    await staleApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "001" });
    const laterGoal = await createGoal({ cwd: repository.root, hostPaths: repository.hostPaths, title: "Later", outcome: "Advance independently.", approval: "Approved." });
    const later = await publishApplication({ root: repository.project, goalId: laterGoal.goal.metadata.goalId, integratedRevision: moved, approval: "Queue later." });
    const candidate = await createSquashCandidate(repository.project, laterGoal.goal.metadata.goalId, moved, moved);
    await publishApplyDecision(repository.project, later, candidate, moved);
    await recoverApplications(repository.project);
  ...
  test("return then landed Goal Change requeues with monotonic IDs and positions without Git mutation", async () => {
    ...
    await returnApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "001", statement: "Resume Goal work." });
    expect(await gitSnapshot(repository)).toEqual(beforeReturn);
    expect(await applicationRequeueEligibility(repository.project, goalId)).toBe("none");
    ...
    const requeued = await queueGoalIntegration({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, approval: "Approve the later Goal revision." });
    ...
    expect((await queuedApplicationHead(repository.project))?.metadata).toMatchObject({ applicationId: "002", queuePosition: 2, integratedRevision: landed });
  });
  ...
  test("refuses stale while review is open, recovers it, then requeues same G and returns", async () => {
    ...
    await expect(staleApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "001" })).rejects.toThrow("every review Ticket to be reported");
    expect(await gitSnapshot(repository)).toEqual(beforeOpenReviewRefusal);
    await recoverApplicationReviewTicket(repository.root, repository.hostPaths, goalId, "001", review.ticket.metadata.ticketId, "Recover the interrupted review.");
    const beforeStale = await gitSnapshot(repository);
    await staleApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "001" });
    expect(await gitSnapshot(repository)).toEqual(beforeStale);
    const requeued = await queueGoalIntegration({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, approval: "Requeue unchanged G after stale." });
    expect(requeued).toMatchObject({ applicationId: "002", queuePosition: 2, integratedRevision: base });
    await recordImplementation(repository, goalId, "002", moved, base, true);
    await recordReview(repository, goalId, "002", moved, base);
    const beforeReturn = await gitSnapshot(repository);
    await returnApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "002", statement: "Return the requeued Candidate." });
    expect(await gitSnapshot(repository)).toEqual(beforeReturn);
    expect(await applicationRequeueEligibility(repository.project, goalId)).toBe("none");
    const status = await deriveGoalStatus(repository.root, repository.hostPaths, goalId);
  ...
  test("refusal and cleanup preconditions leave every Git projection and resolution evidence unchanged", async () => {
    ...
    await expect(staleApplication({ cwd: unresolved.repository.root, hostPaths: unresolved.repository.hostPaths, goalId: unresolved.goalId, applicationId: "001" })).rejects.toThrow("main to differ");
    await expect(returnApplication({ cwd: unresolved.repository.root, hostPaths: unresolved.repository.hostPaths, goalId: unresolved.goalId, applicationId: "001", statement: "   " })).rejects.toThrow("must not be blank");
    await expect(returnApplication({ cwd: unresolved.repository.root, hostPaths: unresolved.repository.hostPaths, goalId: unresolved.goalId, applicationId: "001", statement: "No completed review exists." })).rejects.toThrow("completed highest review");
    expect(await gitSnapshot(unresolved.repository)).toEqual(before);
    expect(await loadApplicationResolutionIfPresent(unresolved.repository.project, unresolved.goalId, "001")).toBeUndefined();
    const later = await publishApplication({ root: unresolved.repository.project, goalId: unresolved.goalId, integratedRevision: unresolved.base, approval: "Later head refusal." });
    const beforeWrongHead = await gitSnapshot(unresolved.repository);
    await expect(staleApplication({ cwd: unresolved.repository.root, hostPaths: unresolved.repository.hostPaths, goalId: unresolved.goalId, applicationId: later.metadata.applicationId })).rejects.toThrow("exact unresolved FIFO head");
    ...
    await expect(staleApplication({ cwd: unreported.repository.root, hostPaths: unreported.repository.hostPaths, goalId: unreported.goalId, applicationId: "001" })).rejects.toThrow("every implementation Ticket to be reported");
    ...
    const exchange = applicationExchangePath(reviewed.repository.project, { goalId: reviewed.goalId, applicationId: "001", ticketId: "001" });
    await mkdir(exchange, { recursive: true });
    const beforeCleanupRefusal = await gitSnapshot(reviewed.repository);
    await expect(returnApplication({ cwd: reviewed.repository.root, hostPaths: reviewed.repository.hostPaths, goalId: reviewed.goalId, applicationId: "001", statement: "Blocked cleanup." })).rejects.toThrow("healthy cleanup");
    await expect(staleApplication({ cwd: reviewed.repository.root, hostPaths: reviewed.repository.hostPaths, goalId: reviewed.goalId, applicationId: "001" })).rejects.toThrow("healthy cleanup");
    expect(await gitSnapshot(reviewed.repository)).toEqual(beforeCleanupRefusal);
    expect(await loadApplicationResolutionIfPresent(reviewed.repository.project, reviewed.goalId, "001")).toBeUndefined();
    ...
    await expect(returnApplication({ cwd: reviewed.repository.root, hostPaths: reviewed.repository.hostPaths, goalId: reviewed.goalId, applicationId: "001", statement: "Main moved." })).rejects.toThrow("requires main");
...
describe("Application terminal resolution", () => {
  test("return admits a real planner, stale refuses it before Herdr side effects, and active Change recovery continues", async () => {
    ...
    await returnApplication({ cwd: returned.repository.root, hostPaths: returned.repository.hostPaths, goalId: returned.goalId, applicationId: "001", statement: "Planner may resume." });
    const returnCalls: string[] = [];
    const beforePlanner = await gitSnapshot(returned.repository);
    expect((await goalPlannerOperations.startOrReattach({ cwd: returned.repository.root, hostPaths: returned.repository.hostPaths, goalId: returned.goalId, herdr: fakePlanner(returnCalls) })).state).toBe("live");
    ...
    await staleApplication({ cwd: stale.repository.root, hostPaths: stale.repository.hostPaths, goalId: stale.goalId, applicationId: "001" });
    const staleCalls: string[] = [];
    const beforeFrozenPlanner = await gitSnapshot(stale.repository);
    await expect(goalPlannerOperations.startOrReattach({ cwd: stale.repository.root, hostPaths: stale.repository.hostPaths, goalId: stale.goalId, herdr: fakePlanner(staleCalls) })).rejects.toThrow("frozen");
  ...
  test("returns only reviewed exact Candidate, unfreezes planning, and leaves main and Goal ref unchanged", async () => {
    ...
    await writeFile(applicationReviewReportPath(repository.project, goalId, "001", "001"), serializeDocument(reviewReport, "# Review report\n"));
    const beforeResolution = await gitSnapshot(repository);
    const returned = await returnApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "001", statement: "Plan product remediation." });
    expect(await gitSnapshot(repository)).toEqual(beforeResolution);
    expect(returned.resolution.metadata.disposition).toBe("return");
    expect(await repository.git("rev-parse", "main")).toBe(base);
    const status = await deriveGoalStatus(repository.root, repository.hostPaths, goalId); expect(status.frozen).toBe(false);
    await createChange({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, title: "After return", intent: "Advance product work.", rationale: "Return released planning.", acceptanceCriteria: ["Planning can continue."] });
    // A returned historical Application has no runtime ownership and cannot
    // prevent ordinary active-Change reconciliation.
    await expect(reconcileGoal({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, recoverApplications: false })).resolves.toMatchObject({ goalId });
    await expect(returnApplication({ cwd: repository.root, hostPaths: repository.hostPaths, goalId, applicationId: "001", statement: "Again." })).rejects.toThrow("exact unresolved FIFO head");
  });
});
