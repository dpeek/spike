...
const policy = z.object({ isolation: z.enum(["workspace", "container"]), credentialGrants: z.array(z.string().min(1)) }).strict();
...
function applicationDirectory(root: ProjectPaths, goalId: string, applicationId: string) { return dirname(applicationPath(root, goalId, applicationId)); }
function ticketsDirectory(root: ProjectPaths, goalId: string, applicationId: string) { return join(applicationDirectory(root, goalId, applicationId), "tickets"); }
export function applicationTicketPath(root: ProjectPaths, goalId: string, applicationId: string, ticketId: string) { return join(ticketsDirectory(root, goalId, applicationId), ticketId, "ticket.md"); }
export function applicationReportPath(root: ProjectPaths, goalId: string, applicationId: string, ticketId: string) { return join(ticketsDirectory(root, goalId, applicationId), ticketId, "report.md"); }
...
export function applicationExchangePath(project: ProjectPaths, identity: ApplicationTicketIdentity) { return join(project.controlRoot, "exchange", "goals", identity.goalId, "applications", identity.applicationId, "tickets", identity.ticketId); }
...
async function ids(root: ProjectPaths, goalId: string, applicationId: string) { return (await listDirectoryNames(root.controlRoot, ticketsDirectory(root, goalId, applicationId))).filter((id) => sequenceIdPattern.test(id)).sort(); }
...
export async function loadApplicationTicket(root: ProjectPaths, goalId: string, applicationId: string, ticketId: string): Promise<ApplicationTicket> {
  const doc = await loadApplicationTicketDocument(root.controlRoot, applicationTicketPath(root, goalId, applicationId, ticketId)); const metadata = doc.metadata;
...
export async function listApplicationTicketIds(root: ProjectPaths, goalId: string, applicationId: string) { const result: string[] = []; for (const id of await ids(root, goalId, applicationId)) if (await documentExists(root.controlRoot, applicationTicketPath(root, goalId, applicationId, id))) { await loadApplicationTicket(root, goalId, applicationId, id); result.push(id); } return result; }
export async function loadApplicationReportIfPresent(root: ProjectPaths, goalId: string, applicationId: string, ticketId: string): Promise<ApplicationReport | undefined> {
  const path = applicationReportPath(root, goalId, applicationId, ticketId); if (!(await documentExists(root.controlRoot, path))) return undefined;
  const doc = await readDocument(root.controlRoot, path); const metadata = applicationReportSchema.parse(doc.metadata);
...
async function integration(root: ProjectPaths, base: string, target: string, goal: string): Promise<{ classification: "clean"; tree: string } | { classification: "conflict"; evidence: string }> {
  ...
  const bases = (await git(root.root, ["merge-base", "--all", target, goal])).split("\n").filter(Boolean);
  ...
  const child = Bun.spawn(["git", "-C", root.root, "merge-tree", "--write-tree", target, goal], { stdin: "ignore", stdout: "pipe", stderr: "pipe" });
...
export type IssueApplicationTicketInput = ApplicationTicketIdentity & { cwd: string; hostPaths: HostPaths; instruction: string; executionPolicy?: Partial<z.infer<typeof policy>>; model?: string; thinking?: ThinkingLevel; now?: Date };
...
export async function issueApplicationTicket(input: Omit<IssueApplicationTicketInput, "ticketId">): Promise<{ root: string; ticket: ApplicationTicket }> {
  ...
  const application = await loadApplication(repository, input.goalId, input.applicationId);
  const existingIds = await listApplicationTicketIds(repository, input.goalId, input.applicationId);
  for (const id of existingIds) if ((await loadApplicationReportIfPresent(repository, input.goalId, input.applicationId, id)) === undefined) throw new Error(`Application ${input.goalId}/${input.applicationId} already has open Ticket ${id}`);
  const warnings = await applicationCleanupWarnings(repository, input.goalId, input.applicationId); if (warnings.length) throw new Error(`Application issuance blocked by cleanup health: ${warnings.join("; ")}`);
  const currentMain = await main(repository);
  const first = existingIds.length === 0 ? undefined : await loadApplicationTicket(repository, input.goalId, input.applicationId, existingIds[0]!);
  ...
  const latestCandidate = await (async () => {
    for (const id of [...existingIds].reverse()) {
      const report = await loadApplicationReportIfPresent(repository, input.goalId, input.applicationId, id);
  ...
  if (remediation) {
    ...
    for (const ticketId of existingIds) {
      const ticket = await loadApplicationTicket(repository, input.goalId, input.applicationId, ticketId);
    ...
    const goal = await (await import("./goal.ts")).loadGoal(repository, input.goalId);
  ...
  const result = remediation ? undefined : await integration(repository, mergeBase, targetRevision, goalRevision);
  ...
  await installImmutable(repository.controlRoot, applicationTicketPath(repository, input.goalId, input.applicationId, ticketId), serializeDocument(metadata, document));
...
async function removeApplicationExchange(root: ProjectPaths, identity: ApplicationTicketIdentity): Promise<void> {
...
export async function applicationCleanupWarnings(root: ProjectPaths, goalId: string, applicationId: string): Promise<string[]> {
  ...
  for (const ticketId of await listApplicationTicketIds(root, goalId, applicationId)) {
    ...
    const workerPath = join(root.controlRoot, "runtime", "application-workers", "goals", goalId, "applications", applicationId, "tickets", ticketId, "worker.md");
    ...
    const retained = await git(root.root, ["show-ref", "--verify", "--quiet", ref]).then(() => true).catch(() => false);
    if (report?.metadata.outcome === "completed") {
      if (!retained || report.metadata.candidateRevision !== await git(root.root, ["rev-parse", ref])) warnings.push(`Application retention ref for Ticket ${ticketId} is missing or mismatched`);
  ...
  const quarantines = await git(root.root, ["for-each-ref", "--format=%(refname)", `refs/spike/quarantine/goals/${goalId}/applications/${applicationId}/`]);
...
async function applicationBundle(root: ProjectPaths, ticket: ApplicationTicket, path: string) {
  ...
  try {
    for (const [ref, value] of refs) await git(root.root, ["update-ref", ref, value, "0".repeat(value.length)]);
    await git(root.root, ["bundle", "create", temporary, ...refs.map(([ref]) => ref)]);
    ...
    await git(root.root, ["bundle", "verify", temporary]);
    const heads = (await git(root.root, ["bundle", "list-heads", temporary])).split("\n").filter(Boolean).map((line) => line.split(/\s+/, 2));
  ...
  } finally { await Promise.all(refs.map(([ref]) => git(root.root, ["update-ref", "-d", ref]).catch(() => undefined))); await rm(temporary, { force: true }); }
}
export async function prepareApplicationTicketExchange(root: ProjectPaths, identity: ApplicationTicketIdentity) {
  ...
  if (await exists(exchange)) {
    ...
    await git(root.root, ["bundle", "verify", join(input, "repository.bundle")]);
  ...
  await ensureWorkflowDirectory(root.controlRoot, input);
  await installImmutable(root.controlRoot, join(input, "ticket.md"), serializeDocument(ticket.metadata, ticket.body));
  await installImmutable(root.controlRoot, join(input, "context.md"), serializeDocument({ kind: "application-ticket-context", ...identity, inputRevision: ticket.metadata.inputRevision, targetRevision: ticket.metadata.targetRevision, goalRevision: ticket.metadata.goalRevision, mergeBase: ticket.metadata.mergeBase, integration: ticket.metadata.integration.classification }, `# Application implementation worker context\n\nStart from exact \`${ticket.metadata.inputRevision}\`. The input bundle exposes only declared target M, Goal G, merge base B, and input refs. Worker output is untrusted; write artifacts only below \`SPIKE_OUTPUT_DIR/artifacts/\`. Complete through the exact Application Ticket worker completion surface; the completion payload contains non-blank summary, verification, assumptions, limitations, risks, follow-up, and declared artifacts. Do not create a Report or retention ref.\n`));
  await applicationBundle(root, ticket, join(input, "repository.bundle")); await Promise.all([chmod(join(input, "ticket.md"), 0o400), chmod(join(input, "context.md"), 0o400), chmod(input, 0o500)]); await ensureWorkflowDirectory(root.controlRoot, output);
  return { ...identity, inputDirectory: input, outputDirectory: output };
}
...
export type PublishApplicationReportInput = ApplicationTicketIdentity & { cwd: string; hostPaths: HostPaths; execution: { adapter: string; isolation: "workspace" | "container"; worker: string; model: string; thinking: z.infer<typeof applicationTicketSchema>["thinking"]; startedAt: string; finishedAt: string }; message: { summary: string; body?: string }; now?: Date };
export async function publishApplicationImplementationReport(input: PublishApplicationReportInput): Promise<{ root: string; report: ApplicationReport }> {
  const repository = await discoverRepository(input.cwd, input.hostPaths); const ticket = await loadApplicationTicket(repository, input.goalId, input.applicationId, input.ticketId); const path = applicationReportPath(repository, input.goalId, input.applicationId, input.ticketId);
  if ((await main(repository)) !== ticket.metadata.targetRevision) throw new Error(`Application target mismatch: main differs from pinned target ${ticket.metadata.targetRevision}`);
  if (await documentExists(repository.controlRoot, path)) throw new Error("immutable Application Report already exists");
  ...
  const recorded = await (await import("./application-worker.ts")).loadFinishedApplicationWorker(repository, input);
  ...
  const output = join(applicationExchangePath(repository, input), "output"); const document = await readDocument(repository.controlRoot, join(output, "submission.md")); const accepted = submission.parse(document.metadata);
  ...
  if (accepted.outcome !== "completed") {
    ...
    await installImmutable(repository.controlRoot, path, serializeDocument(metadata, document.body));
    await (await import("./application-worker.ts")).cleanupApplicationWorker(repository, input).catch(() => undefined);
  ...
  try { await git(repository.root, ["fetch", "--quiet", "--no-tags", bundle, `${advertised}:${quarantine}`]); const worker = await git(repository.root, ["rev-parse", "--verify", `${quarantine}^{commit}`]); if (worker !== accepted.workerRevision) throw new Error("imported Application worker revision mismatches Submission"); const tree = await git(repository.root, ["rev-parse", "--verify", `${worker}^{tree}`]); const summary = text(input.message.summary, "Application Candidate summary"); if (summary.includes("\n")) throw new Error("Application Candidate summary must be one line"); const candidate = await git(repository.root, ["commit-tree", tree, "-p", ticket.metadata.targetRevision, "-m", `${summary}\n\nSpike-Goal-Id: ${input.goalId}`]); const metadata = applicationReportSchema.parse({ kind: "application-report", goalId: input.goalId, applicationId: input.applicationId, ticketId: input.ticketId, role: "implement", outcome: "completed", publishedAt: (input.now ?? new Date()).toISOString(), targetRevision: ticket.metadata.targetRevision, goalRevision: ticket.metadata.goalRevision, mergeBase: ticket.metadata.mergeBase, integrationClassification: ticket.metadata.integration.classification, inputRevision: ticket.metadata.inputRevision, workerRevision: worker, candidateRevision: candidate, artifacts, execution: exactExecution });
    ...
    if ((await main(repository)) !== ticket.metadata.targetRevision) throw new Error(`Application target mismatch: main differs from pinned target ${ticket.metadata.targetRevision}`);
    const report = { metadata, body: document.body }; await installImmutable(repository.controlRoot, path, serializeDocument(metadata, document.body));
...
export async function publishApplicationPartialReport(input: ApplicationTicketIdentity & { cwd: string; hostPaths: HostPaths; execution: PublishApplicationReportInput["execution"]; reason: string; now?: Date }): Promise<{ root: string; report: ApplicationReport }> {
  const repository = await discoverRepository(input.cwd, input.hostPaths), ticket = await loadApplicationTicket(repository, input.goalId, input.applicationId, input.ticketId), path = applicationReportPath(repository, input.goalId, input.applicationId, input.ticketId);
  if ((await main(repository)) !== ticket.metadata.targetRevision) throw new Error(`Application target mismatch: main differs from pinned target ${ticket.metadata.targetRevision}`);
  if (await documentExists(repository.controlRoot, path)) throw new Error("immutable Application Report already exists");
  ...
  await installImmutable(repository.controlRoot, path, serializeDocument(metadata, `# Application Ticket partial\n\n## Reason\n\n${reason}\n`));
  await (await import("./application-worker.ts")).cleanupApplicationWorker(repository, input).catch(() => undefined);
...
export async function deriveApplicationChurn(root: ProjectPaths, goalId: string, applicationId: string): Promise<ApplicationChurnWarning[]> {
...
export async function deriveApplicationStatus(cwd: string, hostPaths: HostPaths, goalId: string, applicationId: string) {
  const repository = await discoverRepository(cwd, hostPaths);
  const application = await loadApplication(repository, goalId, applicationId);
  let resolution: "return" | "stale" | "malformed" | null = null;
  try { resolution = (await loadApplicationResolutionIfPresent(repository, goalId, applicationId))?.metadata.disposition ?? null; }
  catch { resolution = "malformed"; }
  const ticketIds = await listApplicationTicketIds(repository, goalId, applicationId);
  const reports = await Promise.all(ticketIds.map(ticketId => loadApplicationReportIfPresent(repository, goalId, applicationId, ticketId)));
  ...
  const first = ticketIds.length ? await loadApplicationTicket(repository, goalId, applicationId, ticketIds[0]!) : undefined;
  const pinned = first?.metadata.targetRevision ?? null;
  let current: string | null = null;
  try { current = await main(repository); } catch { /* status remains inspectable */ }
  const decision = await (await import("./application.ts")).loadApplicationDecisionIfPresent(repository, goalId, applicationId);
  const evidenceState = await (await import("./application.ts")).applicationState(repository, { ...application, ...(decision === undefined ? {} : { decision }) });
  const head = await queuedApplicationHead(repository);
  const review = await (await import("./application-review.ts")).deriveApplicationReviewStatus(cwd, hostPaths, goalId, applicationId);
  const cleanupWarnings = await applicationCleanupWarnings(repository, goalId, applicationId);
  const worker = open === null ? null : await (await import("./application-worker.ts")).observeApplicationWorker(repository, { goalId, applicationId, ticketId: open });
  return {
    ...
    churnWarnings: await deriveApplicationChurn(repository, goalId, applicationId), worker, review,
  };
}
/** Retryable operational cleanup does not publish or rewrite any Application evidence. */
export async function cleanupApplicationTicketRuntime(cwd: string, hostPaths: HostPaths, goalId: string, applicationId: string, ticketId: string): Promise<{ root: string }> {
  const repository = await discoverRepository(cwd, hostPaths);
  await removeApplicationExchange(repository, { goalId, applicationId, ticketId });
  await (await import("./application-worker.ts")).cleanupApplicationWorker(repository, { goalId, applicationId, ticketId });
...
export async function recoverApplicationTicket(cwd: string, hostPaths: HostPaths, goalId: string, applicationId: string, ticketId: string, reason = "Supervisor recovery interrupted the open Application Ticket.") {
  const repository = await discoverRepository(cwd, hostPaths);
  const ticket = await loadApplicationTicket(repository, goalId, applicationId, ticketId);
  const identity = { goalId, applicationId, ticketId };
  const path = applicationReportPath(repository, goalId, applicationId, ticketId);
  ...
  await adapter.stop(repository, identity);
  await adapter.finalize(repository, identity);
  await adapter.forget(repository, identity);
  ...
  if (!(await documentExists(repository.controlRoot, path))) {
    ...
    await installImmutable(repository.controlRoot, path, serializeDocument(metadata, `# Application Ticket interrupted\n\n${text(reason, "Interruption reason")}\n`));
  }
  await removeApplicationExchange(repository, identity);
  ...
  const reports = await Promise.all((await listApplicationTicketIds(repository, goalId, applicationId)).map(async (id) => ({ id, report: await loadApplicationReportIfPresent(repository, goalId, applicationId, id) })));
